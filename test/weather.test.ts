import { Condition, Coordinates, Weather, Season, getSeason, oppositeSeason } from '../lib/weather';
...
describe('season tests', () => {
  it('should be summer in South', () => {
    const latitude = -31;
    const xmas = new Date('2015-12-25 05:02:12');
    expect(getSeason(latitude, xmas)).toEqual(Season.SUMMER);
  });

  it('should be winter in North', () => {
    const latitude = 56;
    const xmas = new Date('2015-12-25 05:02:12');
    expect(getSeason(latitude, xmas)).toEqual(Season.WINTER);
  });

  it('should show opposite', () => {
    expect(oppositeSeason(Season.AUTUMN)).toEqual(Season.SPRING);
    expect(oppositeSeason(Season.WINTER)).toEqual(Season.SUMMER);
    expect(oppositeSeason(Season.SPRING)).toEqual(Season.AUTUMN);
    expect(oppositeSeason(Season.SUMMER)).toEqual(Season.WINTER);
  });
});
